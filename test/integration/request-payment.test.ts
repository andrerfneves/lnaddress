...
import { alternateTestNodePubkey, testBolt11Invoice, testNodePubkey } from "../fixtures/bolt11";
...
describe("requestPayment nodePubkeys", () => {
  const nodePayRequest = parsePayRequestResponse({
    tag: "payRequest",
    callback: "https://example.com/callback?k1=abc",
    minSendable: 1000,
    maxSendable: 10_000,
  ...
  test("returns verified nodePubkey metadata when the invoice payee matches", async () => {
    const payment = await requestPayment(nodePayRequest, {
      amountMsat: 2000,
      fetch: async () =>
        jsonResponse({ pr: await testBolt11Invoice(2000, nodePayRequest.metadataHash) }),
    });
    ...
    expect(payment).toMatchObject({
      ...
      nodePubkeyVerification: {
        status: "verified",
        payeeNodeId: testNodePubkey,
        payeeNodeIdSource: "n",
        matchedPubkey: testNodePubkey,
        expectedPubkeys: [testNodePubkey],
      },
    });
  });

  test("uses signature recovery when the invoice omits the n field", async () => {
    const payment = await requestPayment(nodePayRequest, {
      amountMsat: 2000,
      fetch: async () =>
        jsonResponse({
          pr: await testBolt11Invoice(2000, nodePayRequest.metadataHash, { omitPayeeNode: true }),
        }),
    });
  ...
  test("returns a non-blocking mismatch warning by default", async () => {
    const payment = await requestPayment(nodePayRequest, {
      amountMsat: 2000,
      fetch: async () =>
        jsonResponse({
          pr: await testBolt11Invoice(2000, nodePayRequest.metadataHash, { signer: "alternate" }),
        }),
    });
    ...
    expect(payment).toMatchObject({
      ...
      nodePubkeyVerification: {
        status: "mismatch",
        payeeNodeId: alternateTestNodePubkey,
        payeeNodeIdSource: "n",
        expectedPubkeys: [testNodePubkey],
      },
    });
    if (payment.type === "bolt11") {
      expect(payment.nodePubkeyVerification?.status).toBe("mismatch");
      if (payment.nodePubkeyVerification?.status === "mismatch") {
        expect(payment.nodePubkeyVerification.warning).toMatch(
          /not generated by an expected node/i,
        );
      }
    }
  });

  test("can enforce nodePubkeys for strict callers", async () => {
    await expect(
      requestPayment(nodePayRequest, {
        amountMsat: 2000,
    ...
    ).rejects.toThrow(NodePubkeyMismatchError);
  });

  test("can disable nodePubkey comparison while keeping other BOLT11 validation", async () => {
    const payment = await requestPayment(nodePayRequest, {
      amountMsat: 2000,
      nodePubkeyPolicy: "off",
      fetch: async () =>
        jsonResponse({
          pr: await testBolt11Invoice(2000, nodePayRequest.metadataHash, { signer: "alternate" }),
        }),
    ...
    expect(payment).toMatchObject({ type: "bolt11" });
    if (payment.type === "bolt11") {
      expect(payment.nodePubkeyVerification).toBeUndefined();
    }
  });
});
